...
!function() {
    ...
    layer.validate = function(layero, callBack){  //弹窗里的验证
        var elem = layero.find('input');
        var regExp = {
            require : /^.*[^\s]+.*$/,
            number : /^[0-9]+$/
        };
        for(var i = 0; i < elem.length; i++){
            ...
            if($(ei).attr('data-validate-require') == 'true'){   //是否为空
                if(!regExp.require.test(eiv)){
                    layer.msg($(ei).attr('data-validate-require-info'),{time:1500});
                    return false;
                }
            }
            if($(ei).attr('data-validate-type') == 'number'){   //是否为数字
                if(!regExp.number.test(eiv)){
                    layer.msg($(ei).attr('data-validate-type-info'),{time:1500});
                    return false;
                }
            }
        }
